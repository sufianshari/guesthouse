            ...
            elseif($mau_ke == "delete"){
                ...
                if ($row) {
                    ...
                    if(!empty($row->pic_halaman)){
                        ...
                        //lokasi file secara spesifik
                        $file = './uploads/foto_galeri/'.$row->pic_halaman;
                        if (!unlink($file)){
                            $this->session->set_flashdata('message', 'File tidak dapat dihapus.');
                            redirect(site_url('dashboard/halaman'));
                        } else {
...
//                        'file_name' =>$nmfile,
                    ...
                    $this->upload->do_upload($this->upload->file_name);
    ...
    public function kamar()
    {
        if (!$this->ion_auth->logged_in())
        {
            ...
            elseif($mau_ke == "update"){
                ...
                $row = $this->admin->get_by_id_kamar($idu);
    ...
    public function reservation()
    {
        ...
        if (!$this->ion_auth->logged_in())
        {
            ...
            if($mau_ke == "update"){
                ...
                $row = $this->admin->get_by_id_reservation($idu);
            ...
            elseif($mau_ke == "delete"){
                ...
                if ($row) {
                    $this->admin->delete_reservation($idu);
                    $this->session->set_flashdata('message', 'Hapus data berhasil.');
                    redirect(site_url('dashboard/reservation'));
                } else {
                    $this->session->set_flashdata('message', 'Data tidak ditemukan.');
